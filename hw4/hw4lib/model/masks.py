...
def PadMask(padded_input, input_lengths):
    ...
    indices = torch.arange(seq_len, device=padded_input.device)
    lengths_unsqueezed = input_lengths.unsqueeze(1)
    mask = indices >= lengths_unsqueezed
...
def CausalMask(padded_input):
    ...
    seq_len = padded_input.size(1)
    mask = torch.triu(
        torch.ones(seq_len, seq_len, device=padded_input.device, dtype=torch.bool), 
        diagonal=1
    )
