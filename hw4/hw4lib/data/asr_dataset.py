...
class ASRDataset(Dataset):
    def __init__(
        ...
        # TODO: Take subset
        subset_size      = self.config.get('subset_size', -1)
        if subset_size > 0:
        ...
        for i in tqdm(range(self.length)):
            ...
            if self.partition != "test-clean":
                ...
                transcript_array = np.load(self.text_files[i])
    ...
    def collate_fn(self, batch) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        feat_lengths = torch.LongTensor([sample[0].shape[1] for sample in batch]) # B
        ...
        padded_feats = pad_sequence(batch_feats, batch_first=True, padding_value=0.0) # B x T_max x F
        ...
        if self.partition != "test-clean":
            ...
            batch_shifted      = [sample[1] for sample in batch] # List of (T,) tensors
            batch_golden       = [sample[2] for sample in batch] # List of (T,) tensors
            ...
            padded_shifted     = pad_sequence(batch_shifted, batch_first=True, padding_value=self.pad_token) # B x T_text_max
            padded_golden      = pad_sequence(batch_golden, batch_first=True, padding_value=self.pad_token) # B x T_text_max
